...
public class UserSecurityImpl implements UserSecurity {
    ...
    @Autowired
    revoblog.repository.userRepository userRepository;
    @Autowired
   ...
   public boolean handSignUp(WebRequest request, HttpServletRequest req, HttpServletResponse res) {
        ...
        if (connectionFromSession != null) {
            ...
            person u = userRepository.findByEmail(userProfile.getEmail()).map(user -> user).orElseGet(() ->
                            userRepository.save(new person().setEmail(userProfile.getEmail()).setPassword(encoder.encode(UUID.randomUUID().toString())).setUserName(
                                    (userProfile.getUsername()!=null)?userProfile.getUsername():userProfile.getFirstName()+" "+userProfile.getLastName()).setImgPath(connectionFromSession.getImageUrl()))
            );
